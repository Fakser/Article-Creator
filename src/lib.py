...
def get_urls(topic, number_of_articles = 10, article_randomness = 0.2):
    ...
    while number_of_proper_articles < number_of_articles:
        ...
        for url in urls:
            if url in new_url or new_url in url or 'youtube' in new_url or '.pdf' in new_url or 1/(edit_distance(new_url, url)+1) > 0.05 or uniform() < article_randomness:
                bad_url = True
                break
        if bad_url == False:
            urls.append(new_url)
...
def get_and_clean_response(urls):
...
def download_images(images):
    ...
    if 'photos' not in list(os.listdir('./')):
        os.mkdir('./photos')
...
def get_cleaned_articles(articles, max_word_len, change_to_synonym_chance, sentence_quality):
...
def create_random_article(cleaned_articles, delete_unnecesarry = True):
    # random new article
...
def get_analytics(random_article):
